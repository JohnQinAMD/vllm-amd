...
def test_fp8_cache_keeps_bf16_query_for_backend_without_quant_input(
    ...
    impl = SimpleNamespace(
        use_quantized_query_input=Mock(return_value=False),
    ...
    result = mla.MultiHeadLatentAttention._decode_concat_cache(
        layer, ql_nope, q_pe, kv_c, k_pe, positions, cos_sin, slots, object()
    )
...
def test_fp8_cache_preserves_quantized_query_fast_path(
    ...
    impl = SimpleNamespace(
        use_quantized_query_input=Mock(return_value=True),
        do_kv_cache_update=Mock(),
    )
    ...
    result = mla.MultiHeadLatentAttention._decode_concat_cache(
        layer, ql_nope, q_pe, kv_c, k_pe, None, None, slots, object()
    )
