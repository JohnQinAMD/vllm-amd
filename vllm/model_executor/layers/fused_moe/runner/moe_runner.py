...
class MoERunner(MoERunnerInterface):
    ...
    def _apply_quant_method(
        ...
        if self.routed_experts.quant_method.is_monolithic:
            ...
            prepared = (
                self.prepared_moe_backend.prepare(
                    hidden_states,
                    router_logits,
                    input_ids,
                    self.router,
                ...
                if self.prepared_moe_backend is not None
                else None
            )
            if prepared is not None:
                if self.router.eplb_state is not None:
                    raise AssertionError("prepared MoE routing cannot bypass EPLB")
                fused_out = self.prepared_moe_backend.consume(
                    hidden_states,
                    self.routed_experts,
                    prepared,
                )
