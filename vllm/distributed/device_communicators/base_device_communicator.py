...
class DeviceCommunicatorBase:
    ...
    def all_reduce_dual(
        ...
        """Fallback implementation using one concatenated all-reduce."""
        left_numel = left.numel()
        combined = torch.cat((left.reshape(-1), right.reshape(-1)))
        reduced = self.all_reduce(combined)
        left_output, right_output = reduced.split(
            (left_numel, right.numel()),
        )
        return left_output.view_as(left), right_output.view_as(right)
